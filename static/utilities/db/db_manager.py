...
class DBManager:
    ...
    def create_new_customer(self, fullname, phone, anotherphonenumber, City, Address, four ):

        query = "INSERT INTO customers( customer_name, customer_phone, customer_extraphone, customer_city, customer_address, employee_id) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % (
            fullname, phone, anotherphonenumber, City, Address, four)
        #self.interact_db(query, 'commit')
        self.commit(query)
        return
    ...
    def get_orders(self):
        query = "select orders.order_id, customers.customer_name, customers.customer_city, orders.order_date, orders.order_price from orders inner join customers on orders.customer_phone=customers.customer_phone  "
    ...
    def get_ordersdetails(self, order_id):
        query = "select orders.order_id, customers.customer_name, customers.customer_city, orders.order_date, orders.order_price  from orders inner join customers " \
    ...
    def get_ordersbyshop(self, shop_id):
        query = "select orders.order_id, customers.customer_name, customers.customer_city, orders.order_date, orders.order_price, employees.shop_id from orders inner join customers on orders.customer_phone=customers.customer_phone inner join employees on customers.employee_id=employees.employee_id "
        query_result =self.fetch(query)
        for que in query_result:
